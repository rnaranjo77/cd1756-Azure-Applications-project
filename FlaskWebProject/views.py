...
# Build image source URL from config
...
@app.route('/post/<int:id>', methods=['GET', 'POST'])  # Fixed HTML escape
@login_required
def post(id):
...
def authorized():
    ...
    if request.args.get('code'):
        ...
        if not isinstance(result, dict) or "error" in result:
            return render_template("auth_error.html", result=result if isinstance(result, dict) else {"error": "unknown"})
        session["user"] = result.get("id_token_claims")
        # Log in as admin user after MS sign-in
